...
import { LuArrowLeft, LuPlus, LuLoader, LuBrain, LuSquareCheck, LuGitPullRequest, LuPlay, LuTrash2, LuFileCode, LuSparkles } from 'react-icons/lu';
...
export const ProjectDetailPage = () => {
  ...
  const fetchProjectData = useCallback(async () => {
    try {
      ...
      const prsRes = await api.get('/projects/' + projectId + '/prs');
  ...
  const handleCreateTask = async () => {
    ...
    try {
      ...
      setTasks(prev => [response.data].concat(prev));
  ...
  const handleExecuteTask = async (taskId) => {
    ...
    try {
      ...
      setTasks(prev => prev.map(t => t.id === taskId ? { ...t, status: 'completed', ai_response: response.data.ai_response } : t));
        ...
        {techStack.length > 0 && (
          <div className="flex flex-wrap gap-2 mb-6">
            {techStack.map((tech, idx) => (
              <Badge key={idx} variant="outline" className="gap-1">{tech}</Badge>
            ))}
        ...
        <Tabs defaultValue="overview" className="space-y-6">
          ...
          <TabsContent value="overview">
            <div className="grid gap-6 lg:grid-cols-2">
              <Card>
                ...
                <CardContent>
                  {projectSummary ? (
                    <div className="whitespace-pre-wrap text-sm text-muted-foreground">{projectSummary}</div>
                  ) : (
          ...
          <TabsContent value="tasks">
            <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                  <div><CardTitle className="text-lg">Tasks</CardTitle><CardDescription>Create tasks for AI to implement</CardDescription></div>
                  <Dialog open={taskDialogOpen} onOpenChange={setTaskDialogOpen}>
                    <DialogTrigger asChild>
                      <Button className="gap-2" data-testid="add-task-btn"><LuPlus className="h-4 w-4" />Add Task</Button>
                    </DialogTrigger>
                    <DialogContent>
                      <DialogHeader><DialogTitle>Create New Task</DialogTitle><DialogDescription>Describe the feature or bug fix</DialogDescription></DialogHeader>
                      <div className="space-y-4 py-4">
                        ...
                        <div className="space-y-2"><label className="text-sm font-medium">Description</label><Textarea placeholder="Implementation details..." value={newTaskDescription} onChange={(e) => setNewTaskDescription(e.target.value)} rows={4} data-testid="task-description-input" /></div>
                        <div className="space-y-2"><label className="text-sm font-medium">Priority</label><Select value={newTaskPriority} onValueChange={setNewTaskPriority}><SelectTrigger data-testid="task-priority-select"><SelectValue /></SelectTrigger><SelectContent><SelectItem value="low">Low</SelectItem><SelectItem value="medium">Medium</SelectItem><SelectItem value="high">High</SelectItem></SelectContent></Select></div>
                      </div>
                      <DialogFooter><Button variant="outline" onClick={() => setTaskDialogOpen(false)}>Cancel</Button><Button onClick={handleCreateTask} disabled={!newTaskTitle.trim() || creatingTask} data-testid="create-task-submit">{creatingTask && <LuLoader className="h-4 w-4 animate-spin mr-2" />}Create Task</Button></DialogFooter>
                    </DialogContent>
                  </Dialog>
                </div>
              </CardHeader>
              <CardContent>
                {tasks.length > 0 ? (
                  <div className="space-y-3">
                    {tasks.map((task, idx) => (
                      <div key={idx} className="p-4 rounded-lg border border-border hover:border-primary/30 transition-colors" data-testid={'task-' + task.id}>
                          ...
                          <div className="flex items-center gap-2">
                            {task.status === 'pending' && (
                              <Button size="sm" onClick={() => handleExecuteTask(task.id)} disabled={executingTaskId === task.id} className="gap-1" data-testid={'execute-task-' + task.id}>
                                {executingTaskId === task.id ? <LuLoader className="h-4 w-4 animate-spin" /> : <LuPlay className="h-4 w-4" />}Execute
                              </Button>
                            )}
                            <Button size="sm" variant="ghost" onClick={() => handleDeleteTask(task.id)} data-testid={'delete-task-' + task.id}><LuTrash2 className="h-4 w-4" /></Button>
                          </div>
                        </div>
                      </div>
                    ))}
          ...
          <TabsContent value="prs">
            <Card>
              <CardHeader><CardTitle className="text-lg">Pull Requests</CardTitle><CardDescription>AI-generated pull requests for completed tasks</CardDescription></CardHeader>
              <CardContent>
                {prs.length > 0 ? (
                  <div className="space-y-3">
                    {prs.map((pr, idx) => (
                      <div key={idx} className="p-4 rounded-lg border border-border hover:border-primary/30 transition-colors">
                        <div className="flex items-start justify-between">
                          <div>
                            ...
                            {pr.github_pr_url && <a href={pr.github_pr_url} target="_blank" rel="noopener noreferrer" className="text-sm text-primary hover:underline mt-1 inline-block">View on GitHub →</a>}
                          </div>
